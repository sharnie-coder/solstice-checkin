...
function renderAttendee(attendee) {
  ...
  if (attendee.status === 'CHECKED_IN') {
    ...
    showNotice(
      checkinNotice,
      'This attendee has already checked in. A second badge cannot be printed.',
      'notice-error'
    );
  } else {
...
checkinBtn.addEventListener('click', async () => {
  ...
  setStatusBadge('status-printing', 'Printing');
  ...
  try {
    ...
    if (!response.ok || !data.success) {
      // Printing failed (or attendee was already checked in by the time
      // this request landed). The attendee stays NOT_CHECKED_IN.
      currentAttendee = data.attendee || currentAttendee;

      if (currentAttendee.status === 'CHECKED_IN') {
        // Someone else already checked this person in - refresh fully.
        renderAttendee(currentAttendee);
        return;
      }

      setStatusBadge('status-error', 'Error');
      showNotice(checkinNotice, data.error || 'Badge printing failed. Please try again.', 'notice-error');
      checkinBtn.disabled = false;
    ...
    setStatusBadge('status-checked-in', 'Checked In');
    badgeIdRow.hidden = false;
    attendeeBadgeId.textContent = currentAttendee.badgeId || '—';
    showNotice(checkinNotice, '✓ Checked In Successfully', 'notice-success');
    checkinBtn.classList.add('hidden');
  } catch (err) {
    printingIndicator.classList.add('hidden');
    setStatusBadge('status-error', 'Error');
